...
export default function SeatingClient({ locale }: SeatingClientProps) {
            ...
            const defaultTables: Table[] = [
              {
                ...
                y: 100,
                shape: 'rectangle',
                ...calculateTableSize(10, 'rectangle')
              },
              {
                id: '2', 
                name: 'Family Table',
  ...
  const calculateTableSize = (capacity: number, shape: 'round' | 'rectangle' | 'square') => {
    ...
    switch (shape) {
      case 'round':
        return { radius: baseSize / 2 };
      case 'rectangle':
        return { width: baseSize * 1.4, height: baseSize * 0.6 };
      case 'square':
  ...
  const handleEventChange = (eventName: string) => {
    // Save current arrangement before switching
  ...
  const saveCurrentArrangement = async () => {
    ...
    try {
      ...
      await setDoc(seatingDocRef, filterUndefined({
        eventId: selectedEvent,
        eventName: selectedEvent,
        tables: tables,
  ...
  const handleDragStart = (e: React.DragEvent, guest: Guest) => {
    console.log('Drag start for guest:', guest.name, guest.id);
  ...
  const handleDrop = (e: React.DragEvent, tableId: string) => {
    ...
    if (guest) {
      console.log('Adding guest to table:', guest.name, '->', tableId);
  ...
  const unassignedGuests = guests.filter(guest => {
    // Check if guest's group is invited to the selected event
    const guestGroup = guest.groupId ? groups[guest.groupId] : null;
    const isInvitedToEvent = !selectedEvent || (guestGroup && guestGroup.events.includes(selectedEvent));
    const isNotAssignedToTable = !tables.some(table => table.guests.some(tGuest => tGuest.id === guest.id));
    return isInvitedToEvent && isNotAssignedToTable;
  });

  // Get unique categories for filter dropdown from tags array
  ...
  const filteredGuests = guests.filter(guest => {
    ...
    // Filter by selected event - check if guest's group is invited to this event
    const guestGroup = guest.groupId ? groups[guest.groupId] : null;
    const isInvitedToEvent = !selectedEvent || (guestGroup && guestGroup.events.includes(selectedEvent));
    
    if (showUnassignedOnly) {
      return matchesSearch && isUnassigned && matchesCategory && isInvitedToEvent;
    }
    
    return matchesSearch && matchesCategory && isInvitedToEvent;
  });
      ...
      <div className="flex h-screen">
        ...
        <div className="w-80 bg-white shadow-lg overflow-hidden flex flex-col">
          <div className="p-4 border-b">
            <h2 className="text-lg font-semibold mb-4">{getLocalizedText(locale, 'guest_list')}</h2>
            ...
            <div className="space-y-2">
              {filteredGuests.map((guest) => {
                ...
                return (
                  ...
                  >
                    <div className="font-medium pr-6">{guest.name}</div>
