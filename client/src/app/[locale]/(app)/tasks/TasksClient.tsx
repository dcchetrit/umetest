    ...
    const loadData = async () => {
      try {
        ...
        const displayTasks: Task[] = tasksData.map(task => ({
          ...task,
          dueDate: task.dueDate ? (typeof task.dueDate === 'string' ? task.dueDate.split('T')[0] : task.dueDate.toISOString().split('T')[0]) : undefined,
              ...
              <div className="bg-white rounded-lg shadow p-6">
                ...
                <div className="relative">
                  ...
                  <div className="space-y-8">
                    {(() => {
                      // Group tasks by month for better timeline visualization
                      const tasksByMonth: { [key: string]: Task[] } = {};
                      const sortedTasks = [...filteredAndSortedTasks].sort((a, b) => {
                        if (!a.dueDate && !b.dueDate) return 0;
                        if (!a.dueDate) return 1;
                        if (!b.dueDate) return -1;
                        return new Date(a.dueDate).getTime() - new Date(b.dueDate).getTime();
                          ...
                          <div key={monthKey} className="relative">
                            ...
                            <div className="ml-16 space-y-3">
                              {tasks.map(task => {
                                ...
                                const isDueSoon = dueDate && dueDate >= today && 
                                    ...
                                    <div className="flex justify-between items-start">
                                      <div className="flex-1">
                                        <div className="flex items-center gap-2 mb-2">
                                          ...
                                          {isDueSoon && !isOverdue && (
                                            <span className="text-yellow-600 text-xs font-bold">DUE SOON</span>
                                          )}
                                        </div>
