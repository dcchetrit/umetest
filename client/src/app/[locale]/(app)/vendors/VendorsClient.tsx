...
import { getBudgetCategories, saveBudgetCategory, BudgetCategory } from '@/services/budgetCategoryService';
import { saveExpense, getEvents } from '@/services/budgetService';
import { ExpenseEntry } from '@/types/budget';
...
export default function VendorsClient({ locale }: VendorsClientProps) {
  ...
  const loadData = async () => {
    ...
    try {
      ...
      // Load vendors, budget categories, and events in parallel
      const [vendorsData, categoriesData, eventsData] = await Promise.all([
        loadVendors(),
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (editingVendor) {
        // Check if status is changing to 'booked'
        const isChangingToBooked = editingVendor.status !== 'booked' && formData.status === 'booked';
        
        // Calculate payment status
        const paymentStatus = calculatePaymentStatus(
          formData.finalCost || 0,
          formData.depositPaid || 0,
          formData.finalPaymentDue
        );
        ...
        // Create expense entry if status changed to booked
        if (isChangingToBooked) {
          await createExpenseFromVendor({ ...formData, paymentStatus } as Vendor);
        }
        ...
        // Add new vendor
        const vendorsRef = collection(db, 'couples', user.uid, 'vendors');
        const docRef = await addDoc(vendorsRef, filterUndefined({
          ...formData,
          eventId: formData.eventId || null,
          paymentStatus,
          createdAt: now,
        ...
        // Create expense entry if new vendor is booked
        if (formData.status === 'booked') {
          await createExpenseFromVendor({ ...formData, id: docRef.id, paymentStatus } as Vendor);
        }
  ...
  const handleCreateCategory = async (e: React.FormEvent) => {
    ...
    try {
      await saveBudgetCategory(user.uid, {
  ...
  const calculatePaymentStatus = (finalCost: number = 0, depositPaid: number = 0, finalPaymentDue?: string): 'paid' | 'due' | 'overdue' => {
    if (finalCost === depositPaid) {
      return 'paid';
    }
    
    if (finalCost > depositPaid) {
      if (finalPaymentDue) {
        const dueDate = new Date(finalPaymentDue);
        const now = new Date();
        if (dueDate < now) {
          return 'overdue';
        }
  ...
  const createExpenseFromVendor = async (vendor: Vendor) => {
    if (!user) return;
    
    try {
      // Find the matching budget category
      const matchingCategory = budgetCategories.find(cat => cat.name === vendor.category);
      if (!matchingCategory) {
        console.warn(`No matching budget category found for vendor category: ${vendor.category}`);
        return;
      }
      
      // Get the event name from eventId
      const eventName = vendor.eventId ? events.find(e => e.id === vendor.eventId)?.name : undefined;
      
      // Calculate proper payment status for the expense
      const quotedPrice = vendor.finalCost || vendor.estimatedCost || 0;
      const amountPaid = vendor.depositPaid || 0;
      let expensePaymentStatus: 'paid' | 'due' | 'overdue' = 'due';
      
      if (quotedPrice === amountPaid) {
        expensePaymentStatus = 'paid';
      } else if (quotedPrice > amountPaid) {
        if (vendor.finalPaymentDue) {
          const dueDate = new Date(vendor.finalPaymentDue);
          const now = new Date();
          expensePaymentStatus = dueDate < now ? 'overdue' : 'due';
        } else {
          expensePaymentStatus = 'due';
        }
      }
      
      // Create expense entry from vendor data
      const expenseData: Omit<ExpenseEntry, 'id' | 'createdAt' | 'updatedAt'> = {
        categoryId: matchingCategory.id,
        vendorName: vendor.name,
        quotedPrice,
        amountPaid,
        paymentStatus: expensePaymentStatus,
        paymentDueDate: vendor.finalPaymentDue ? new Date(vendor.finalPaymentDue) : null,
        notes: `Auto-created from booked vendor: ${vendor.notes || ''}`.trim(),
        event: eventName
      };
      
      await saveExpense(user.uid, expenseData);
      console.log(`Created expense entry for booked vendor: ${vendor.name} (Event: ${eventName || 'No event'})`);
    } catch (error) {
      console.error('Error creating expense from vendor:', error);
    }
  };
          ...
          {filteredVendors.map((vendor) => (
            <div key={vendor.id} className="bg-white rounded-lg shadow p-6 hover:shadow-lg transition-shadow">
              ...
              <div className="space-y-2 text-sm mb-4">
                ...
                {vendor.eventId && (
                  <p><strong>Event:</strong> {events.find(e => e.id === vendor.eventId)?.name || 'Unknown Event'}</p>
                )}
                ...
                {vendor.paymentStatus && (vendor.finalCost || vendor.estimatedCost) && (
                  <div className="flex items-center">
                    <strong className="mr-2">Payment:</strong>
                    <span className={`inline-flex px-2 py-1 text-xs font-semibold rounded-full ${
                      vendor.paymentStatus === 'paid' ? 'bg-green-100 text-green-800' :
                      vendor.paymentStatus === 'overdue' ? 'bg-red-100 text-red-800' :
                      'bg-yellow-100 text-yellow-800'
                    }`}>
                      {vendor.paymentStatus === 'paid' ? 'Paid' :
                       vendor.paymentStatus === 'overdue' ? 'Overdue' :
                       'Due'}
                    </span>
                  </div>
                )}
                  ...
                  <div>
                    ...
                    >
                      ...
                      {events.map((event, index) => (
                        <option key={event.id || `event-${index}`} value={event.id || ''}>{event.name}</option>
                      ))}
                    </select>
                  </div>
                  <div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                  <div>
                    ...
                    <input
                      ...
                      onChange={(e) => {
                        ...
                        const paymentStatus = calculatePaymentStatus(
                          formData.finalCost || estimatedCost,
                          formData.depositPaid || 0,
                          formData.finalPaymentDue
                        );
                        setFormData({ ...formData, estimatedCost, paymentStatus });
                      }}
                      className="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500"
                    />
                  </div>
                  <div>
                    ...
                    <input
                      ...
                      onChange={(e) => {
                        ...
                        setFormData({ ...formData, finalCost, paymentStatus });
                      }}
                      className="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500"
                    />
                  </div>
                  <div>
                    ...
                    <input
                      ...
                      onChange={(e) => {
                        const depositPaid = parseFloat(e.target.value) || 0;
                        const paymentStatus = calculatePaymentStatus(
                          formData.finalCost || formData.estimatedCost || 0,
                          depositPaid,
                          formData.finalPaymentDue
                        );
                        setFormData({ ...formData, depositPaid, paymentStatus });
                      }}
                      className="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500"
                    />
                  </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  ...
                  <div>
                    ...
                    <input
                      ...
                      onChange={(e) => {
                        const finalPaymentDue = e.target.value;
                        const paymentStatus = calculatePaymentStatus(
                          formData.finalCost || formData.estimatedCost || 0,
                          formData.depositPaid || 0,
                          finalPaymentDue
                        );
                        setFormData({ ...formData, finalPaymentDue, paymentStatus });
                      }}
                      className="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500"
                    />
                  </div>
                ...
                <div className="bg-gray-50 p-4 rounded-lg">
                  ...
                  <div className="flex items-center">
                    <span className={`inline-flex px-3 py-2 text-sm font-semibold rounded-full ${
                      ...
                      formData.paymentStatus === 'overdue' ? 'bg-red-100 text-red-800' :
                      'bg-yellow-100 text-yellow-800'
                    }`}>
                      {formData.paymentStatus === 'paid' ? 'Payment Made' :
                       formData.paymentStatus === 'overdue' ? 'Payment Overdue' :
                       'Payment Due'}
                    </span>
                    <span className="ml-3 text-sm text-gray-600">
                      {formData.finalCost || formData.estimatedCost ? 
                        `$${(formData.depositPaid || 0).toLocaleString()} of $${((formData.finalCost || formData.estimatedCost) || 0).toLocaleString()} paid` :
                        'Enter amounts to see payment status'
                      }
                    </span>
                  </div>
                </div>
            ...
            <div className="p-6">
              ...
              <div className="space-y-4">
                <div className="grid grid-cols-2 gap-4">
                  ...
                  {viewingVendor.eventId && (
                    <div>
                      <strong>Event:</strong> {events.find(e => e.id === viewingVendor.eventId)?.name || 'Unknown Event'}
                    </div>
                  )}
                  {viewingVendor.paymentStatus && (viewingVendor.finalCost || viewingVendor.estimatedCost) && (
                    <div>
                      <strong>Payment Status:</strong>
                      <span className={`ml-2 inline-flex px-2 py-1 text-xs font-semibold rounded-full ${
                        viewingVendor.paymentStatus === 'paid' ? 'bg-green-100 text-green-800' :
                        viewingVendor.paymentStatus === 'overdue' ? 'bg-red-100 text-red-800' :
                        'bg-yellow-100 text-yellow-800'
                      }`}>
                        {viewingVendor.paymentStatus === 'paid' ? 'Payment Made' :
                         viewingVendor.paymentStatus === 'overdue' ? 'Payment Overdue' :
                         'Payment Due'}
                      </span>
                    </div>
                  )}
